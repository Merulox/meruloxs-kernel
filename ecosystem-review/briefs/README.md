...
| BX-05a | `blocked` | Make crm.db sole lead source-of-truth (retire md reads) | **2026-06-14:** audit done + read-path repoints made, but blocked on one scope wall — `review-request` *writes* `crm.md`, and fixing it needs a crm.db **write** (outside this read-only `[DATA]` brief). **DECISION NEEDED (PO/architect):** authorize a `review_requests` crm.db write-path (new mini-task), or stop recording review requests to md. Rest of the audit is complete | ~/scripts (non-live md readers) | [DATA] reviewer; DO NOT TOUCH reply-agent/sms-inbox/sms-webhook | BX-02 |

...
| AP-17 | `briefed` | Generalize owned-path detection (stop dropping `web/`,`docs/`) | **Caught on S-19:** applyCommit's `pathLike` only matches `src/`/`api/` → `web/src/main.jsx` silently uncommitted (architect committed it 0fa7866). Any non-aperture repo layout loses owned files. Broaden the regex | aperture launch-codex.ts (`ownedPathTokens`) | parse-only | AP-12, AP-15 |

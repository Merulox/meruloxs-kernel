...
| WEB-03 | `briefed` | Refresh merulox.com /projects + /employers | Roster is stale/wrong (SYNTRA listed as "discovery pipeline" not the live EDC retailer; Boréal "paused" but resumed; public /projects empty). Update to current truth + create public /projects; preview on dev.merulox.com, PO promotes prod | ~/website/src/pages/{projects,employers} + data | `[DEPLOY]` preview only; prod promote PO-gated | — |
...
| BX-05a | `briefed` | Make crm.db sole lead source-of-truth (retire md reads) | Post-investigation: acute issues already resolved (decoys gone, crm.md absent, outreach-batch on crm.db). Remaining = audit ~18 md-referencing scripts, repoint reads-as-truth to crm.db; leads.md stays as scraper→importer staging; live SMS services fenced off | ~/scripts (non-live md readers) | [DATA] reviewer; DO NOT TOUCH reply-agent/sms-inbox/sms-webhook | BX-02 |
...
| AP-19 | `briefed` | Add a Boréal category to the /tasks board | BX-* tasks render mixed into Ecosystem; Boréal is its own workstream (cash loop, own repo). Split BX-* into their own panel — display-only, README stays the source | aperture tasks.ts + new BorealPanel.tsx + Taskboard.tsx | read-only UI | AP-12, AP-15 |
